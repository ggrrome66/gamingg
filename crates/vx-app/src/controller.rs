...
/// Per-frame mouse look for walk mode, plus the eye height the save system
/// and fly-to-walk handoff share. The walking physics itself lives in
/// `movement.rs`, on the fixed tick, where determinism can hold.
#[derive(Debug, Clone, Copy)]
pub struct WalkController {
    /// Radians of rotation per pixel of mouse movement.
    pub sensitivity: f32,
    /// Standing eye height above the feet, for camera handoffs.
    pub eye_height: f32,
}
...
impl WalkController {
    /// Apply this frame's mouse motion to the camera. Cosmetic: the movement
    /// simulation reads only the quantised yaw from the sampled command.
    pub fn look(&self, camera: &mut Camera, input: &mut InputState) {
        let (dx, dy) = input.take_mouse_delta();
        if input.mouse_captured {
            camera.yaw += dx * self.sensitivity;
            camera.pitch -= dy * self.sensitivity;
...
mod walk_tests {
    ...
    #[test]
    fn look_only_turns_while_captured_and_consumes_motion() {
        ...
        input.mouse_captured = false;
        input.add_mouse_delta(500.0, 500.0);
        controller.look(&mut camera, &mut input);
        assert_eq!(camera.yaw, 0.0, "turned while the mouse was free");

        input.mouse_captured = true;
        input.add_mouse_delta(100.0, -40.0);
        controller.look(&mut camera, &mut input);
        assert!(camera.yaw > 0.0);
        assert!(camera.pitch > 0.0, "mouse up should pitch up");

        // Motion was consumed, not banked.
        controller.look(&mut camera, &mut input);
        let yaw_after = camera.yaw;
        controller.look(&mut camera, &mut input);
        assert_eq!(camera.yaw, yaw_after);
    }
}
